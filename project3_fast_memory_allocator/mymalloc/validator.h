...
static int add_range(const malloc_impl_t* impl, range_t** ranges, char* lo,
  ...
  if (!IS_ALIGNED(lo)) {
    printf("Payload address is not aligned!\n");
    return 0;
  }

  // The payload must lie within the extent of the heap
  // TODO(project3): YOUR CODE HERE
  if (!((char*)impl->heap_lo() <= lo && (char*)impl->heap_hi() >= hi)) {
    printf("Payload does not lie within extent of the heap.\n");
    return 0;
  }
  ...
  for (p = *ranges; p != NULL; p = p->next){
    if (!(p->hi < lo || hi < p->lo)) {
      printf("Payload overlaps with another payload.\n");
      return 0;
    }
  }
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    switch (trace->ops[i].type) {
      case ALLOC:  // malloc
        ...
        // fill with bottom byte of size
        uint8_t bottom_byte = 'A'; //size & 0xff;
        memset(p, bottom_byte, size);
      ...
      case REALLOC:  // realloc
        ...
        for (int i=0; i<oldsize; i++){
          if (newp[i] != bottom_byte) {
	          printf("New block doesn't match data from old block.\n");
            return 0;
          }
        }
        memset(newp, bottom_byte, size);
